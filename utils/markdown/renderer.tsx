...
export default class Renderer extends marked.Renderer {
  ...
  public link(href: string, title: string, text: string, isUrl = false) {
    ...
    if (href.includes('localhost')) {
      const postId = href.slice(href.slice(0, href.lastIndexOf('/') + 1).lastIndexOf('/') + 1);;
      let message = "";
      let channelName = "";
      let userName = "";
      ...
      $.ajax({
        type: "GET",
        async: false,
        url: `http://localhost:8065/api/v4/posts/${postId}`,
        beforeSend: function (xhr) {
          xhr.setRequestHeader("Authorization", "Bearer heyxfa3bh3ddfec8cki5h9khpc")
        },
        success: function (data) {
          ...
          $.ajax({
            type: "GET",
            async: false,
            url: `http://localhost:8065/api/v4/channels/${data.channel_id}`,
            beforeSend: function (xhr) {
              xhr.setRequestHeader("Authorization", "Bearer heyxfa3bh3ddfec8cki5h9khpc")
            },
            success: function (data2) {
              channelName = data2.display_name;
              isChannelPrivate = data2.type === "P";
              if (!isChannelPrivate) {
                $.ajax({
                  type: "GET",
                  async: false,
                  url: `http://localhost:8065/api/v4/users/${data.user_id}`,
                  beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer heyxfa3bh3ddfec8cki5h9khpc")
                  },
      ...
      if (isChannelPrivate) {
        output += `<span data-for='ethan-private'>`;
      } else {
        output += `<span data-for='ethan-public' data-username='${userName}'
        data-channelname='${channelName}'
        data-hack='${message.includes("handsome")}'
        data-message='${message}'>`;
      }
    }

    output += `<a class="theme markdown__link`;
    ...
    // output += `<div class="__react_component_tooltip place-left type-dark" id="ethan" data-id="tooltip" style="left: 3px; top: 9px;"><span>ETHAN WAS HERE!!!</span></div>`;

    // if (href.includes('localhost')) {
    //   output += `<div data-test="foo" data-text="ETHAN WAS HERE!!!"></div>`;
    // }

    // output += `<ReactTooltip id='ethan' place='left' type='dark' effect='solid'>`;
    // output += '<span>ETHAN WAS HERE!!!</span></ReactTooltip>';
    return output;
  }
